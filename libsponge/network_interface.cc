...
void NetworkInterface::send_datagram(const InternetDatagram &dgram, const Address &next_hop) {
    ...
    frame.header().src = _ethernet_address; // the sender
...
void NetworkInterface::_retransmission_arp_frame() {
    ...
    if (!_pending_flag || (_pending_flag && _timer - _pending_timer >= 5000)) {
        ...
        msg.target_ethernet_address = {0,0,0,0,0,0}; // INIT NULL
...
optional<InternetDatagram> NetworkInterface::recv_frame(const EthernetFrame &frame) {
    ...
    if (frame.header().type == EthernetHeader::TYPE_ARP) {
        ...
        _table[ip].ttl = _timer + 30 * 1000; // 30secs
        ...
        while (!_frames_waiting.empty()) {
            ...
            if (_table.count(node.ip) && _timer <= _table[node.ip].ttl) {
                ...
                _frames_waiting.pop();
                _frames_out.push(std::move(node.frame));
            } else {
                break;
            }
        }
...
bool NetworkInterface::_ethernet_address_equal(const EthernetAddress& addr1, const EthernetAddress& addr2) const {
    for (int i = 0; i < 6; ++i) {
        if (addr1[i] != addr2[i]) {
            return false;
        }
